...
export async function randomTicket(): Promise<Ticket> {
  ...
  const { data } = await api.post<Ticket>("/api/simulator/random-ticket");
  return data;
}
...
export async function forceFailure(): Promise<Ticket> {
  ...
  const { data } = await api.post<Ticket>("/api/simulator/force-failure");
  return data;
}
