...
class EventBus:
    ...
    def get_incomplete_resolved_tickets(self) -> Optional["StreamEvent"]:
        event = self._redis_client.xreadgroup(
            consumername="python-worker-1",
            groupname="TICKET_WORKERS",
            streams={"TICKETS:RESOLVED_STREAM": "0"},
            count=1,
        )
        if event is None:
            return
    ...
    def listen_incomplete_tickets(self) -> Optional["StreamEvent"]:
        event = self._redis_client.xreadgroup(
            groupname="TICKET_WORKERS",
            consumername="python-worker-1",
            streams={"TICKETS_STREAM": "0"},
            count=1,
        )
